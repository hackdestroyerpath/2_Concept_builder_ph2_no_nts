...
P2R5 выполняет не выборочную правку примеров, а полный обход зарегистрированных Markdown-файлов из `Registry/page_registry.jsonl`. JSONL-реестры используются как источник путей и машинная metadata; проверка читаемой прозы относится к Markdown-страницам и человекочитаемым строкам evidence.
...
| Группа | Исправление |
|---|---|
| `Issues/*/README.md` | английские заголовки `Provenance`, `Scope`, `Evidence`, `Cleanup`, `Closure rule` переведены; статусные литералы сохранены. |
| `Protocols/*.md` | заголовки, матрицы, table headers и readable prose переведены; имена файлов, поля схем и статусные литералы сохранены. |
| `Templates/` | навигационные подписи, роли артефактов, правила шаблонов и safety/check/gate prose переведены. |
...
| `README.md`, `Instructions/`, `Inbox/`, `Registry/structure.md` | остаточные `production`, `state`, `validation`, `issue`, `full sweep`, `Governance`, `Evidence`, `staging` и аналогичные читаемые фразы заменены русскими эквивалентами или вынесены в допустимые технические литералы. |
| `Validation/` | языковая проверка теперь фиксирует полный sweep P2R5, а не только примеры P2R4. |
...
| Идентификаторы задач и сегментов | `CB-P2`, `CB-009`, `P2R5`, `D-001`…`D-063` | точные идентификаторы tracking/evidence |
