...
validation_ref: p2r5_branch_readback_and_main_readback_after_fast_forward
...
|---|---|
| `README.md` и `Instructions/` | читаемая проза переведена; product/mode literals сохранены |
| `Protocols/` | полный набор зарегистрированных протоколов перечитан; английские заголовки, table prose и readable prose заменены русскими формулировками |
| `Issues/` | артефакты `CB-002`…`CB-009`, `CB-P2` и index страницы согласованы с русской читаемой прозой |
...
| `Validation/sync_report.md` | этот отчёт фиксирует ветку, изменённые зоны, перечитывание и отсутствие блокеров |
...
| `Issues/issue_registry.jsonl` | строка `CB-P2` сохраняет P2R5, 63/63/0, `remaining: []` и residual list `[]` |
...
| `Concepts/smoke/output.txt` | `404` / absent | `absent_with_evidence` |
| `Concepts/smoke/e.txt` | `404` / absent | `absent_with_evidence` |
