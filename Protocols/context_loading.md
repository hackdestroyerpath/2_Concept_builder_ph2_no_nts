...
[← Реестр протоколов](protocol_index.md) | [Архитектура состояния](state_architecture.md) | [Маршрутизация режимов](mode_routing.md)
...
6. Прочитать активный `issue`, registry/events и validation anchors, если действие меняет production-файлы.
...
| `local` | Локальный объект работы. | Активный `issue`, выбранная концепция, local registry. | Не читать чужой объект без ссылки. |
...
| `excluded` | Не загружать без отдельной причины. | patch-handoff, Phase 1 audit, prompt-ы, checkpoint-и, исходный handoff-архив. | Не писать в production repo. |
...
Минимальный набор перед работой с концепцией:
...
| Scenario | Вход | Ожидаемый context set | Результат P2-003 |
|---|---|---|---|
| Service patch | `CB-P2`, target `Protocols/state_architecture.md` | README, service state, protocol index, state/context/mode/marker protocols, issue registry/events, CB-P2, final/sync | маршрут подтверждён; scope остаётся `Service Mode` |
| Execution fixture read | `Concepts/smoke`, export readiness | README, execution state, Concepts entry, smoke README/local registry, execution bootstrap, concept export | маршрут подтверждён; запись в service files запрещена без transfer |
| Mixed request | service protocol + selected concept content | сначала `mode_routing.md`, затем разделение на service patch и execution task | смешанный write без transfer блокируется |
