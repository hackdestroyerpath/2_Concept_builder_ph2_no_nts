...
| [`page_registry.jsonl`](page_registry.jsonl) | локальный реестр с metadata | нет строк только с путём |
...
| [`process.md`](process.md) | рабочий процесс | проверочные крючки присутствуют |
...
Все изменения этой концепции выполняются через `Execution Mode`, связанную задачу, обновление локального реестра и `validation_protocol.md`. Бесхозные файлы запрещены; удалённый путь заранее записывается в локальный реестр или доказательство.